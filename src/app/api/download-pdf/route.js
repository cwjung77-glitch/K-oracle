...
export async function POST(req) {
  try {
    ...
    const addNewPage = () => {
      ...
      // Subtle border
      doc.rect(20, 20, doc.page.width - 40, doc.page.height - 40).lineWidth(1).strokeColor('#222222').stroke();
    };

    // Helper for Headers
    const addHeader = (title, subtitle) => {
      doc.fillColor(primaryColor).fontSize(20).text(title, 50, 60, { align: 'left' });
      doc.fillColor('#A1A1AA').fontSize(11).text(subtitle, 50, 90, { align: 'left' });
    ...
    if (type === 'saju') {
      ...
      doc.rect(15, 15, doc.page.width - 30, doc.page.height - 30).lineWidth(2).strokeColor(primaryColor).stroke();
      
      doc.fillColor(primaryColor).fontSize(48).text('K-ORACLE', 0, 300, { align: 'center', characterSpacing: 10 });
      doc.fillColor(textColor).fontSize(20).text(isEs ? 'DECODIFICA TU DESTINO Y AURA' : 'DECODE YOUR DESTINY & AURA', { align: 'center', characterSpacing: 2 });
      doc.moveDown(1.5);
      doc.fillColor('#888888').fontSize(12).text(isEs ? 'Reporte Maestro 2027 (Edición VIP)' : '2027 Master Report (VIP Edition)', { align: 'center' });
      
      doc.fillColor('#555555').fontSize(10).text('© 2027 K-ORACLE AI. STRICTLY CONFIDENTIAL.', 0, 750, { align: 'center', characterSpacing: 4 });

      // ---------------- PAGE 2: SOUL BLUEPRINT ----------------
      addNewPage();
      addHeader(isEs ? '1. El Matriz de tu Alma' : '1. The Soul Matrix', isEs ? 'El núcleo de tu identidad cósmica' : 'The core of your cosmic identity');
      doc.fillColor(textColor).fontSize(12).text(data.content || (isEs ? 'Tu energía se alinea con la fuerza del Fuego. Iluminas la oscuridad pero debes tener cuidado de no quemarte.' : 'Your energy aligns with the force of Fire. You illuminate the darkness but must be careful not to burn out.'), { lineGap: 8, align: 'justify' });

      // ---------------- PAGE 3: 5 ELEMENTS RADAR CHART ----------------
      addNewPage();
      addHeader(isEs ? '2. Balance de los 5 Elementos' : '2. The 5 Elements Balance', isEs ? 'Madera, Fuego, Tierra, Metal, Agua' : 'Wood, Fire, Earth, Metal, Water');
      ...
      for (let i = 0; i < 5; i++) {
        ...
        doc.fillColor(primaryColor).fontSize(10).text(elements[i], lx - 30, ly - 5, { align: 'center', width: 60 });
      }
      ...
      doc.fillColor(textColor).fontSize(11).text(isEs ? 'Tu elemento dominante es la Tierra, dándote estabilidad financiera.' : 'Your dominant element is Earth, giving you financial stability.', 50, 630, { align: 'center' });
      ...
      const gX = 50, gY = 500, gW = doc.page.width - 100, gH = 200;
      ...
      [0, 25, 50, 75, 100].forEach(level => {
        ...
        doc.fillColor('#666').fontSize(8).text(`${level}%`, gX - 35, y - 4, { align: 'right', width: 30 });
      });
      ...
      for (let i = 0; i < 12; i++) {
        ...
        doc.fillColor(textColor).fontSize(9).text(months[i], x - 15, gY + 10, { align: 'center', width: 30 });
      }

      doc.fillColor(textColor).fontSize(11).text(isEs ? 'Agosto a Septiembre marca tu temporada cumbre. Prepárate.' : 'August to September marks your peak season. Prepare for a major event.', 50, 560, { align: 'center' });
      ...
      for (let i = 0; i < 6; i++) {
        ...
        doc.fillColor(textColor).fontSize(12).text(dummyContent, { lineGap: 10, align: 'justify' });
        doc.moveDown();
        doc.fillColor('#A1A1AA').fontSize(12).text(dummyContent, { lineGap: 10, align: 'justify' });
        doc.moveDown();
        doc.fillColor(textColor).fontSize(12).text(dummyContent, { lineGap: 10, align: 'justify' });
      }
      ...
      addNewPage();
      doc.fillColor(textColor).fontSize(20).text('BEAUTY REPORT - UNDER CONSTRUCTION', 50, 100);
    }
