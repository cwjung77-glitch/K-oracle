...
export default function OracleLanding() {
  ...
  const handleLogoClick = () => {
    if (hasPaid) {
      if (window.confirm(lang === 'ko' ? "PDF ÎßàÏä§?∞Ìîå?úÏùÑ ?§Ïö¥Î°úÎìú?òÏÖ®?òÏöî? ÏßÄÍ∏?Î©îÏù∏?ºÎ°ú ?åÏïÑÍ∞ÄÎ©?Î∂ÑÏÑù Í≤∞Í≥ºÍ∞Ä ?ÅÍµ¨?ÅÏúºÎ°???†ú?©Îãà??" : "Did you download your PDF Masterplan? Leaving now will permanently erase your results.")) {
          ...
          {!hasPaid && recentHistory.length > 0 && (
            <div className="w-full max-w-sm mx-auto mb-6">
              <h3 className="text-zinc-300 font-bold text-sm mb-3 flex items-center gap-2 justify-center">
                <Sparkles size={14} /> {lang === 'ko' ? 'ÏµúÍ∑º ?¥Îûå??Í∏∞Î°ù' : 'Recent Readings'}
              </h3>
              <div className="space-y-2">
                {recentHistory.map((item, idx) => (
                  ...
                  >
                    <div>
                      <div className="font-bold text-zinc-200 text-sm">{item.type === 'saju' && item.plan === 'compatibility' ? `${item.name} ?§Ô∏è ${item.idolName}` : item.name}</div>
