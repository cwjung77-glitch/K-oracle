...
export default function SajuCompatibility({ onUnlockPremium }) {
  ...
  const handleAnalyze = () => {
    ...
    const talismans = [
      { text: '?°Ïö¥?¥Ïπò', en: "Protection", type: "classic" },
      { text: "?âÏïàÎ¨¥ÏÇ¨", en: "Peace", type: "classic" },
      { text: "?∏Í∏∞?àÏ†ï", en: "Superstar", type: "kpop" },
      { text: "Îß§Î†•Î∞úÏÇ∞", en: "Potential", type: "kpop" },
      { text: "?¨ÏøµÏ£ºÏùò", en: "Heart Attack", type: "kpop" },
      { text: "?úÏÑ†Í∞ïÌÉà", en: "Eye Catcher", type: "kpop" },
      { text: "Î™ÖÏòà?ÅÏäπ", en: "Honor", type: "classic" },
      { text: "Í¥ëÌÅ¥?±Í≥µ", en: "Fast Click", type: "kpop" },
      { text: "?ïÏßàÎßåÎ†ô", en: "Pro Fan", type: "kpop" },
      { text: "?ºÏ∑®?îÏû•", en: "Growth", type: "classic" },
      { text: "?¨Î¨º??∞ú", en: "Money Rain", type: "classic" },
      { text: "Îß§Î†•ÎßåÏ†ê", en: "Charisma", type: "kpop" },
      { text: "?ÑÍ¥ë?ùÌôî", en: "Speed", type: "classic" },
      { text: "?§Î∞ç?±Í≥µ", en: "Stream", type: "kpop" },
      { text: "Î≥∏Î∞©?¨Ïàò", en: "Live Watch", type: "kpop" }
    ];
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <input type="time" value={time} onChange={(e) => setTime(e.target.value)} style={{ colorScheme: "dark" }} className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-yellow-500" />
                  ...
                  >
                    <option value="female">?Ä Female (?¨ÏÑ±)</option>
                    <option value="male">??Male (?®ÏÑ±)</option>
                  </select>
              </div>
            </div>
                ...
                <div className="space-y-3 animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <select value={customGender} onChange={(e) => setCustomGender(e.target.value)} className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-yellow-500 appearance-none cursor-pointer">
                      <option value="female">?Ä Female</option>
                  ...
                  <div className="inline-block relative mb-10 group" style={{ perspective: '1000px' }}>
                    <div className={`w-56 h-[340px] ${t.paper} rounded-md flex flex-col items-center justify-between relative overflow-hidden ${t.shadow} transition-all duration-500 group-hover:scale-105 group-hover:rotate-2 border-[6px] border-double ${t.border} ${isUltraRare ? 'ring-4 ring-yellow-400 animate-pulse' : ''}`}>
                      ...
                      <div className={`mt-6 w-14 h-14 border-4 ${t.border} rounded-full flex items-center justify-center ${t.ink} font-black text-2xl opacity-90 z-10`}>
                        {userElement === 'Fire' ? '?? : userElement === 'Water' ? 'Ê∞? : userElement === 'Wood' ? '?? : userElement === 'Metal' ? '?? : '??}
                      </div>
                      ...
                      <div id="ig-story-card" className="w-full aspect-[9/16] bg-zinc-950 rounded-3xl border border-zinc-800 p-6 flex flex-col items-center justify-between relative overflow-hidden shadow-2xl">
                        ...
                        <div className="relative z-10 w-full text-center mt-6">
                          ...
                          <div className="text-2xl font-black text-white bg-black/50 py-2 px-4 rounded-full inline-block border border-white/10 backdrop-blur-sm">
                            {userName ? userName.toUpperCase() : 'ME'} ?§Ô∏è {selectedIdol.name}
                          </div>
                        </div>
