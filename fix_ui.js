...
// Use regex to replace the exact block flexibly
content = content.replace(/<div className="bg-black\/50 border border-white\/5 p-5 rounded-2xl">\s*<div className="flex items-center gap-2 mb-3">\s*<Star size=\{16\} className="text-violet-400" \/>\s*<h4 className="font-bold text-zinc-300 text-sm uppercase tracking-widest">\{lang === 'es' \? 'Match de Ídolo' : 'Idol Match'\}<\/h4>\s*<\/div>\s*<p className="text-sm text-zinc-300 leading-relaxed">\s*\{result\.idolMatch\}\s*<\/p>\s*<\/div>\s*<\/div>\s*<\/div>/g, replacement.trim());
