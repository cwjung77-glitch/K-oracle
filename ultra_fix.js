...
// 3. Remove the old Amulet section completely, starting exactly from the comment
// {/* Daily Amulet Freemium Hook */} up to its closing div.
// To be extremely precise, we find the index of the comment and the index of the next "})()}"
const commentIdx = content.indexOf('{/* Daily Amulet Freemium Hook */}');
if(commentIdx > -1) {
    const endSnippet = '})()}\n          </div>';
    const endIdx = content.indexOf(endSnippet, commentIdx);
    if(endIdx > -1) {
        const fullAmuletBlock = content.substring(commentIdx, endIdx + endSnippet.length);
        content = content.replace(fullAmuletBlock, '');
    } else {
        console.log("Could not find end of amulet section.");
    }
} else {
    console.log("Could not find amulet comment.");
}

fs.writeFileSync('src/components/features/DailyFortune.jsx', content, 'utf8');
console.log("All UI fixes perfectly applied!");
