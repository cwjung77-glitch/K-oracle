  ...
  `let response;
    ...
    while (retries > 0) {
      ...
      if (response.status === 429 || response.status >= 500) {
        retries--;
        console.warn(\`[AI Beauty Engine] API Error \${response.status}. Retries left: \${retries}\`);
        if (retries === 0) {
